...
from flask import Blueprint, current_app, jsonify, redirect, render_template, request, session, url_for, send_from_directory
from flask_login import login_required
from .helper_functions import user_permissions
from .models import QueryConfiguration, get_model_by_name
import os
import uuid
...
@login_required
# @user_permissions('Admin')  # Example of how to use the user_permissions decorator.
def site_map():
    """
...
def api_data():
    """
    Returns JSON data for a given query_id. Used by the Fetch API (AJAX) call in data_loader.js.
    ...
    config = QueryConfiguration.query.filter_by(id=query_id).first()
    if not config:
        return jsonify({'items': []}), 404

    filters = config.filters or {}
    columns = config.columns or []
    model_class = get_model_by_name(config.model_name)
    if not model_class:
        return jsonify({'items': []}), 400

    query = model_class.query
    for col_name, val in filters.items():
        query = query.filter(getattr(model_class, col_name) == val)
